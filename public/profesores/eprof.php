...
$id = $_GET['id'];
$miProfesor = (new Profesores)->setId($id)->read();
    ...
    if (is_uploaded_file($_FILES['img']['tmp_name'])) {
        if (isImagen($_FILES['img']['type'])) {
            ...
            if (!move_uploaded_file($_FILES['img']['tmp_name'], trim(dirname(__DIR__, 1) . "/img/$nombreImg"))) {
                ...
                if (!str_contains($miProfesor->img, "default.png")) {
                    unlink(dirname(__DIR__, 1) . "/img/" . basename($miProfesor->img));
                }
            }
        } else {
...
<body style="background-color:#6EAEFF ">
    <h3 class="text-center">Editar Profesor (<?php echo $id ?>)</h3>
    <div class="container mt-2">
        <div class="m-auto bg-gradient bg-danger text-white rounded p-4 shadow-lg" style="width: 48rem;">
            <div class="text-center">
                <img src="<?php echo $miProfesor->img ?>" width="80rem" height="80rem" class="rounded-circle" />
            </div>
            <form action="<?php echo $_SERVER['PHP_SELF']."?id=$id" ?>" method="post" enctype="multipart/form-data">
                <div class="mb-3">
                    <label for="nombreCat" class="form-label">Nombre de Profesor</label>
                    <input type="text" class="form-control" id="nombre" name="nom_prof" value="<?php echo $miProfesor->nom_prof ?>" placeholder="Introduce el nombre aqui" required>
                    <?php
                ...
                <div class="mt-3">
                    ...
                    <input type="number" class="form-control" id="nombre" name="sueldo" value="<?php echo $miProfesor->sueldo ?>" placeholder="El sueldo" required>
                    <?php
                ...
                <div class="mt-3">
                    ...
                    <select class="form-select" aria-label="Default select example" required name="dep_id">
                        ...
                        foreach ($departamentos as $item) {
                            if ($item->id == $miProfesor->dep_id) {
                                echo "<option value='{$item->id}' selected>{$item->nom_dep}</option>";
                            } else {
